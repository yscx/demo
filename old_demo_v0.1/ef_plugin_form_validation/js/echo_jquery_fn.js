...
jQuery.fn.extend({
	ini_astv2_datepicker:function(cur_lang,begin_dateObj,today_dateObj){
		     var $root = jQuery(this);
			 var $date_begin=$root.find('.js_date_begin');
			 var $date_end=$root.find('.js_date_end');
			 var begin_minDate_dateObj = (begin_dateObj.getTime()>today_dateObj.getTime()) ? (today_dateObj) :(begin_dateObj);
			 var end_dateObj ;
			...
			function getFormatDateStr(__dateObj__){
				return ( __dateObj__.getFullYear() + '-' + (__dateObj__.getMonth()+1) + '-' + __dateObj__.getDate() );
			}
			
			if($date_end.attr('value')){
				end_dateObj = new Date(parseDate($date_end.attr('value')));	
				console.log(end_dateObj.getTime());
			}
			 ...
			 switch(cur_lang){
				...
				if( $date_begin.length && $date_end.length ){				
					...
					$date_begin.datepicker('option',{
						'minDate' : begin_minDate_dateObj,
						'maxDate':'+1y'
					});
					
					//初始
					if($date_end.attr('value')){//如果end值已存在，即是event edit page
						$date_end.datepicker('option',{
							'minDate':(new Date( end_dateObj.getTime() - 366*24*3600*1000 ) ),//(new Date( parseDate(date_begin_str)+24*3600*1000 ) ),
							'maxDate':(new Date( end_dateObj.getTime() + 366*24*3600*1000 ) )	
						});
					}else{//如果end值不存在，即是event add page
						$date_end.datepicker('option',{
							'minDate':today_dateObj,//(new Date( parseDate(date_begin_str)+24*3600*1000 ) ),
							'maxDate':(new Date( today_dateObj.getTime() +366*24*3600*1000 ) )	
						});
					}
